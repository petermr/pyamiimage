...
# local
from ..pyimage.ami_graph_all import AmiEdge, AmiGraph, AmiIsland, AmiNode
...
plt.show()"""
        ...
        assert (
            str(graph.nodes[0].keys()) == "dict_keys([AmiEdge.PTS, 'o'])"
        ...
        edges_gen = graph[0][
            2
        ]  # plural because there may be multiple edges between nodes
        assert (
            f"{list(edges_gen)[:1]}" == "[AmiEdge.PTS]"
        )  # pts are connected points in an edge
    ...
    3----2-------------4----5
        ...
        for i, ami_edge in enumerate(ami_edges[:3]):
            tol = 1
            segments = ami_edge.create_line_segments(tolerance=tol)
            logger.debug(
                f"tol {tol} edge {ami_edge.start_id, ami_edge.end_id} segments {len(segments)} -> {segments}"
            )
            assert len(segments) == lengths[i]
            assert ami_edge.start_id == starts[i]
            assert ami_edge.end_id == ends[i]
            assert type(segments) is numpy.ndarray, f"type {type(segments)}"
            assert segments.tolist() == lists[i], f"segments {segments}"
    ...
    def test_islands(self):
        ...
        assert (
            type(connected_components[0]) is set and len(connected_components[0]) == 8
    ...
    def test_get_nodes_with_degree(self):
        ...
        self.assert_degrees(
            ami_graph,
            1,
            [0, 1, 3, 5, 6, 7, 8, 9, 10, 11, 14, 15, 16, 17, 20, 21, 22, 23, 25, 26],
        )
    ...
    def test_bboxes(self):
        ...
        assert (
            str(bbox_list[0]) == "[[661, 863], [82, 102]]"
        ), f"bbox_list[0] is {bbox_list[0]}"
    ...
    def test_plot_lines_with_nodes(self, interact=interactive):
        ...
        if interact:
            # TODO split into line segmentation and plotting
    ...
    def test_extract_raw_image(self):
        ...
        self.display_erode_dilate(
            self.arrows1, self.nx_graph_arrows1, erode=True, dilate=True
        )
    ...
    def test_find_bboxes_with_text(self, interact=interactive):
        ...
        for text_box in text_boxes:
            if TextUtil.is_text_from_tesseract(text_box.text):
                assert (
                    type(text_box) is TextBox
                ), f"cannot add {type(text_box)} as TextBox"
                text_boxes1.append(text_box)
        assert len(text_boxes1) > 0, "require non_zero count of text_boxes"
        logger.info(
            f"{__name__} plotting {len(text_boxes1)} text_boxes of type {type(text_boxes1[0])}"
        )
    ...
    def test_get_nx_edge_lengths_list_for_node(self):
        ...
        expected1 = pytest.approx(
            {
                (24, 21): 30.004166377354995,
                (24, 22): 9.394147114027968,
                (24, 23): 9.394147114027968,
                (24, 25): 12.010412149464313,
            },
            0.001,
        )
        assert {
    ...
    def test_battery1square(self):
        ...
        print(
            "edges[0]",
            list(nx_graph.edges)[0],
            type(list(nx_graph.edges)[0]),
            len(list(nx_graph.edges)[0]),
        )
        print("=======  yyyy  =======")
        print(
            "edges(0, 1)[0]",
            "tuple->",
            list(nx_graph.edges(0, 1))[0],
            type(list(nx_graph.edges(0, 1))[0]),
        )  # start
        print("=======  edges  =======")
    ...
    def assert_degrees(cls, ami_graph, degree, result_nodes):
        ...
        assert (
            nodes == result_nodes
        ), f"nodes of degree {degree} should be {result_nodes}"
    ...
    def test_whole_image(self):
        ...
        islands_big = [
            island
            for island in islands
            if island.get_or_create_bbox().min_dimension() > 20
        ]
    ...
    def test_whole_image1(self):
        ...
        for max_dim in counts_by_maxdim:
            islands_big = AmiIsland.get_islands_with_max_dimension_greater_than(
                max_dim, islands
            )
    ...
    def test_graph_plots(self):
        ...
        assert (
            str(horizontal_lines) == horiz_line_coord_pairs
        ), f"horizontal lines should be {horiz_line_coord_pairs}"
        ...
        assert (
            str(vertical_lines) == vert_line_coord_pairs
        ), f"vertical lines should be {vert_line_coord_pairs}"
        ...
        assert (
            str(non_hv_lines) == non_horvert_node_ids
        ), f"non-axial lines should be {non_hv_lines}"
